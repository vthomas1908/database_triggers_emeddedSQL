...
//allow user to update agent/language/languagerel tables
//and use trigger to update the agent_with_languages table
//calling refresh materialized view
//gcc -std=c99 -L$(pg_config --libdir) -I$(pg_config --includedir) -o task3 task3.c -lpq
...
int main(int argc, char** argv)
{
  ...
  memset(string, '\0', 500);
  strcpy(string, "create materialized view agent_with_languages3 as ");
  ...
  //create the functions that will update the agent_with_languages3 table
  //with a refresh materialized view when agent/languagerel changed
  memset(string, '\0', 500);
  strcpy(string, "create or replace function t3_f_refresh() ");
  strcat(string, "returns trigger as ");
  strcat(string, "$$ begin ");
  strcat(string, "refresh materialized view agent_with_languages3; ");
  ...
  memset(string, '\0', 500);
  strcpy(string, "create trigger t3_t_atrigger ");
  strcat(string, "after update or insert or delete on agent ");
  strcat(string, "for each row execute procedure t3_f_refresh();");
  ...
  memset(string, '\0', 500);
  strcpy(string, "create trigger t3_t_btrigger ");
  strcat(string, "after update or insert or delete on languagerel ");
  strcat(string, "for each row execute procedure t3_f_refresh();");
  ...
  res = PQexec(conn,
    "declare main cursor for select * from agent order by agent_id");
  check_command(conn, res);
  PQclear(res);

  //get count of rows
  res = PQexec(conn, "select count(*) from agent");
  ...
  for(int i = 1; i < rows; i++)
  {
    ...
    memset(string, '\0', 500);
    strcpy(string, "select count(*) from languagerel ");
    ...
    strcat(string, "select lang_id, language from languagerel natural join language");
    strcat(string, " where agent_id = ");
    strcat(string, PQgetvalue(res, 0, 0));
    ...
    if(atoi(choice) == 1)
    {
      ...
      //insert row into agent table and languagerel table
      //trigger will refresh the agent_with_languages3 table
      memset(string, '\0', 500);
      ...
      //since this is a new agent, we need to add to languagerel table too
      memset(string, '\0', 500);
    ...
    if(atoi(choice) == 2)
    {
      //delete current agent from agent (will cascade to languagerel)
      //trigger will refresh materialized view
      memset(string, '\0', 500);
      ...
      //may be used again even if no other agents currently speak
    ...
    if(atoi(choice) == 3)
    {
      ...
      //otherwise add to languagerel, trigger will refresh
      //materialized view
      else
        {
          memset(string, '\0', 500);
    ...
    if(atoi(choice) == 5)
    {
      ...
      while(!update_done)
      {
        ...
        if(atoi(choice) == 2)
        {
          ...
          //prep sql string for agent, trigger to agent_with_languages3
          memset(string, '\0', 500);
        ...
        if(atoi(choice) == 10)
        {
          ...
          strcat(string, lang_id);
          strcat(string,
           " in(select lang_id from languagerel where agent_id = ");
          strcat(string, agent_id);
          strcat(string, ")");
          ...
          else
          {
            ...
            //update in languagerel table, 
            //trigger to agent_with_languages refresh
            memset(string, '\0', 500);
  ...
  res = PQexec(conn, "drop trigger t3_t_btrigger on languagerel");
  check_command(conn, res);
  PQclear(res);

  res = PQexec(conn, "drop trigger t3_t_atrigger on agent");
  check_command(conn, res);
  PQclear(res);

  res = PQexec(conn, "drop function t3_f_refresh()");
  check_command(conn, res);
  PQclear(res);
